    ...
    try:
        ...
        if hasattr(cookie_jar, 'items'):
            ...
            for cookie in cookie_jar:
             ...
             logging.warning(f"Could not determine how to iterate cookies object: {type(cookies)}")
             return
...
async def load_cookies(cookie_file):
    ...
    try:
        ...
        logging.warning(f"Error loading cookies from {cookie_file}: {e}. Ignoring cookies.")
    except Exception as e:
        logging.warning(f"Unexpected error loading cookies from {cookie_file}: {e}. Ignoring cookies.")
    return {}
...
def adaptive_parse_text_and_language(metadata_json):
    ...
    try:
        ...
        if response_container is None:
            logging.error("Could not find 'fetch_query_formulation_metadata_response' container.")
            return None, [], []
                ...
                if isinstance(candidate_iterable, list) and candidate_iterable and isinstance(candidate_iterable[0], list):
                    try:
                        ...
                        if len(first_segment) > 1 and isinstance(first_segment[1], list):
                            if (first_segment[1] and isinstance(first_segment[1][0], list) and
                                len(first_segment[1][0]) > 0 and isinstance(first_segment[1][0][0], list)):
                                segments_iterable = candidate_iterable
                                logging.debug(f"Segments list identified at path ending with {path_name}")
                                break
        ...
        if segments_iterable is None:
            logging.error(f"Could not identify valid text segments list using paths {path_names}.")
            return language, [], []
        ...
        for segment_list in segments_iterable:
            ...
            try:
                if len(segment_list) > 1 and isinstance(segment_list[1], list):
                                ...
                                for word_info in word_list:
                                    try:
                                        if (isinstance(word_info, list) and len(word_info) > 3 and
                                                isinstance(word_info[1], str) and
                                                isinstance(word_info[2], str) and
                                                isinstance(word_info[3], list) and word_info[3] and
                                                isinstance(word_info[3][0], list)):

                                            text = word_info[1]
                                            space_indicator = word_info[2]
                                            bbox = word_info[3][0]

                                            current_block_word_annotations.append({"text": text, "bbox": bbox})
                    ...
                    logging.warning(f"Word groups list structure [1] not found/invalid.")
            except (IndexError, TypeError):
        ...
        logging.error(f"Critical error during adaptive text extraction: {e}", exc_info=True)
...
async def scan_image(image_path):
    ...
    async with httpx.AsyncClient(cookies=loaded_cookies, follow_redirects=True, timeout=timeout, limits=limits, http2=True, verify=True) as client:
        try:
            # --- 1. Upload Image to Lens ---
            logging.debug(f"POST request to {LENS_UPLOAD_ENDPOINT}")
            response_upload = await client.post(LENS_UPLOAD_ENDPOINT, headers=HEADERS, files=files, params=params_upload)
            ...
            try:
                ...
                language, reconstructed_blocks, all_word_annotations = adaptive_parse_text_and_language(metadata_json)
                full_text = "\n".join(reconstructed_blocks)

                result_data = {
                    "text": full_text,
                    "language": language if language else "und",
                    "text_with_coordinates": json.dumps(all_word_annotations, ensure_ascii=False) # JSON as string
                }
                return result_data, metadata_json

            except Exception as e_parse:
                logging.error(f"Error parsing JSON or extracting text: {e_parse}", exc_info=True)
                return None, response_metadata.text

        except httpx.HTTPStatusError as e:
            logging.error(f"HTTP error: {e.response.status_code} for URL {e.request.url}")
...
async def main():
    ...
    logging.info(f"Total scan_image execution time: {end_total_time - start_total_time:.2f} sec.")

    if result_dict:
        print("\n--- Google Lens Scan Result ---")
        print(json.dumps(result_dict, indent=2, ensure_ascii=False)) # Output result as JSON
        print("------------------------------")
    else:
