...
def resize_image(image_path, max_size=(1000, 1000)):
    ...
    try:
        with Image.open(image_path) as img:
            original_size = img.size  # Maintain the dimensions of the original image
            img.thumbnail(max_size)
            if img.mode == 'RGBA':
                img = img.convert('RGB')
            buffer = io.BytesIO()
            img.save(buffer, format="JPEG")
            return buffer.getvalue(), img.size, original_size  # Also return original_size
...
def resize_image_from_buffer(buffer, max_size=(1000, 1000)):
    ...
    try:
        ...
        return output_buffer.getvalue(), img.size, original_size  # Also return original_size
    except (IOError, OSError, ValueError) as e:
