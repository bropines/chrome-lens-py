...
from ..exceptions import LensException, LensConfigError
from ..utils.config_manager import build_app_config, get_default_config_dir, update_config_file_from_cli
from ..utils.general import is_image_file_supported
from ..utils.sharex import copy_to_clipboard
from ..constants import DEFAULT_CONFIG_FILENAME, DEFAULT_CLIENT_REGION, DEFAULT_CLIENT_TIME_ZONE
...
def setup_logging(level_str: str = "WARNING"):
    ...
    logging.basicConfig(
        level=log_level,
        format=log_format,
        handlers=[RichHandler(console=console, show_time=False, show_level=log_level <= logging.INFO, show_path=log_level <= logging.DEBUG, markup=True, rich_tracebacks=True)]
    )
    if log_level > logging.DEBUG: logging.getLogger("httpx").setLevel(logging.WARNING)
...
def print_help():
    ...
    table.add_column(style="green"); table.add_column()
    table.add_row("Usage:", "lens_scan <image_source> [ocr_lang] [options]")
    table.add_row("\n[bold]Arguments:[/bold]")
    table.add_row("  image_source", "Path to an image file or a URL.")
    table.add_row("  ocr_lang", "BCP 47 language code for OCR (e.g., 'en', 'ja'). If omitted, auto-detection is attempted.")
    table.add_row("\n[bold]Translation Options:[/bold]")
    table.add_row("  -t, --translate TARGET_LANG", "Target language for translation (e.g., 'en', 'ru').")
    table.add_row("  --translate-from SOURCE_LANG", "Source language for translation (auto-detected if omitted).")
    table.add_row("  -to, --translate-out FILE_PATH", "Save the image with translated text overlaid.")
    table.add_row("\n[bold]Output and Config Options:[/bold]")
    table.add_row("  --get-coords", "Output recognized words with their coordinates in JSON format.")
    table.add_row("  -sx, --sharex", "Copy the result (translation or OCR) to the clipboard.")
    table.add_row("  --ocr-single-line", "Join all OCR text into a single line (preserves line breaks by default).")
    table.add_row("  --config-file FILE_PATH", "Path to a custom JSON configuration file.")
    table.add_row("  --update-config", "Update the default config file with CLI arguments.")
    ...
    table.add_row("  --proxy URL", "Proxy server URL (e.g., http://user:pass@host:port, socks5://host:port).")
    table.add_row("  --timeout SECONDS", "Request timeout in seconds (default: 60).")
    table.add_row("  --client-region REGION", f"Client region code (default: '{DEFAULT_CLIENT_REGION}').")
    table.add_row("  --client-time-zone TZ", f"Client time zone ID (default: '{DEFAULT_CLIENT_TIME_ZONE}').")
    table.add_row("  -l, --logging-level LEVEL", "Set logging level (DEBUG, INFO, WARNING, ERROR).")
...
async def cli_main():
    ...
    # Positional
    parser.add_argument("image_source", nargs="?", help="Path to the image file or a URL.")
    ...
    # Output & Config
    parser.add_argument("--get-coords", action="store_true", help="Output word coordinates in JSON format.")
    parser.add_argument("-sx", "--sharex", action="store_true")
    parser.add_argument("--ocr-single-line", action="store_false", dest="ocr_preserve_line_breaks", default=None)
    ...
    if not args.image_source:
        console.print("[bold red]Error:[/bold red] The 'image_source' argument is required.\n"); print_help(); sys.exit(1)
    ...
    try:
        ...
        console.print(f"[bold red]Configuration Error:[/bold red] {e}"); sys.exit(1)

    setup_logging(app_config.get("logging_level", "WARNING"))
    
    if os.path.exists(config_file_to_load): logging.info(f"Using config file: {config_file_to_load}")
    elif args.config_file_path_override: logging.warning(f"Specified config file not found: {args.config_file_path_override}")
    
    if not is_image_file_supported(args.image_source):
        console.print(f"[bold red]Error:[/bold red] Source '{args.image_source}' is not a valid URL or supported image file."); sys.exit(1)
    
    if args.update_config:
        if args.config_file_path_override:
            console.print("[bold yellow]Warning:[/bold yellow] --update-config only affects the default config file.")
        else:
            try: update_config_file_from_cli(vars(args), default_config_path)
            except LensConfigError as e: console.print(f"[bold red]Error updating config:[/bold red] {e}")
    ...
    try:
        ...
        if args.get_coords:
            ...
            # Печатаем вручную, без rich.json.JSON
            console.print("[")
            ...
            for i, data in enumerate(word_data):
                ...
                # Отступ + JSON строка + запятая (если нужна)
                line_suffix = "," if i < num_words - 1 else ""
                console.print(f"  {json.dumps(output_obj, ensure_ascii=False)}{line_suffix}")
            console.print("]")
            ...
            if args.sharex:
                ...
                if text_to_copy:
                    if copy_to_clipboard(text_to_copy): 
                        ...
                        console.print("\n[bold red]Failed to copy text. Is 'pyperclip' installed? (`pip install \"chrome-lens-py[clipboard]\"`)[/bold red]")
                else: 
