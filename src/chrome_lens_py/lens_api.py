...
from .text_processing import simplify_output # stitch_* functions are used within simplify_output
from .exceptions import LensAPIError, LensParsingError, LensCookieError, LensImageError, LensError
from .utils import is_url, is_supported_mime # Keep utils
...
class LensAPI:
    def __init__(self, config=None, sleep_time=1000, logging_level=logging.WARNING, rate_limit_rpm=None):
        ...
        if rate_limit_rpm is not None:
            try:
                ...
                if 1 <= rate_limit_rpm_int <= 60: # Allow slightly wider range here, LensCore caps at 40
                    effective_config.setdefault('rate_limiting', {})['max_requests_per_minute'] = rate_limit_rpm_int
                    logging.info(f"API level rate limit override set to {rate_limit_rpm_int} RPM.")
                else:
                    logging.warning(
        ...
        try:
            self.lens = Lens(config=effective_config, sleep_time=sleep_time, logging_level=logging_level)
    ...
    async def _process_single_source(self, image_source):
        ...
        try:
            if isinstance(image_source, Image.Image):
                ...
                image_buffer = image_to_jpeg_buffer(image_source) # Can raise LensImageError
                parsed_data, original_size = await self.lens.scan_by_buffer(image_buffer) # Await async call
            elif isinstance(image_source, np.ndarray):
                logging.debug("Processing NumPy array")
                image_buffer = numpy_array_to_jpeg_buffer(image_source) # Can raise LensImageError
                parsed_data, original_size = await self.lens.scan_by_buffer(image_buffer) # Await async call
            elif isinstance(image_source, str) and is_url(image_source):
                logging.debug(f"Processing URL: {image_source}")
                parsed_data, original_size = await self.lens.scan_by_url(image_source) # Await async call
            elif isinstance(image_source, str) and os.path.isfile(image_source):
                logging.debug(f"Processing file path: {image_source}")
                parsed_data, original_size = await self.lens.scan_by_file(image_source) # Await async call
            elif isinstance(image_source, bytes):
                 logging.debug("Processing raw bytes buffer")
                 # Assume JPEG or let scan_by_buffer handle it
                 parsed_data, original_size = await self.lens.scan_by_buffer(image_source) # Await async call
            else:
                raise LensAPIError(f"Unsupported image source type: {type(image_source)}")
        ...
        # Catch specific errors from Lens or image processing
            ...
            logging.error(f"Unexpected error processing image source: {e}", exc_info=True)
        ...
        if parsed_data is None:
             ...
             raise LensAPIError("Failed to get parsed data from Lens scan, received None.")
    ...
    async def get_full_text(self, image_source):
        ...
        try:
            # Process the source
            parsed_data, _ = await self._process_single_source(image_source) # Don't need original_size here
            ...
            raise
             ...
             raise LensParsingError(f"Parsed data structure missing key for full text: {e}")


    async def get_text_with_coordinates(self, image_source, coordinate_format='percent'):
        """Gets text annotations with coordinates (and angle)."""
        logging.debug(f"get_text_with_coordinates called for source type: {type(image_source)}")
        try:
            # Process source
            parsed_data, original_size = await self._process_single_source(image_source)
            ...
            if 'text_with_coordinates' not in simplified_result:
                 logging.error("Failed to extract 'text_with_coordinates' during simplification.")
                 # Check for underlying error in simplified_result
                 if 'error' in simplified_result:
                      raise LensParsingError(f"Error during simplification: {simplified_result['error']}")
                 else:
                      raise LensParsingError("Unknown error: 'text_with_coordinates' key missing after simplification.")
            ...
            raise
             ...
             raise LensParsingError(f"Simplified data structure missing expected key: {e}")

    async def get_stitched_text_smart(self, image_source):
        """Gets text stitched using the 'smart' (line reconstruction) method."""
        logging.debug(f"get_stitched_text_smart called for source type: {type(image_source)}")
        try:
            # Process source
            parsed_data, original_size = await self._process_single_source(image_source) # Need original size for simplify
            ...
            if 'stitched_text_smart' not in simplified_result:
                 logging.error("Failed to extract 'stitched_text_smart' during simplification.")
                 if 'error' in simplified_result:
                      raise LensParsingError(f"Error during simplification: {simplified_result['error']}")
                 else:
                      raise LensParsingError("Unknown error: 'stitched_text_smart' key missing after simplification.")
    ...
    async def get_stitched_text_sequential(self, image_source):
        ...
        logging.debug(f"get_stitched_text_sequential called for source type: {type(image_source)}")
        try:
             # Process source
            parsed_data, original_size = await self._process_single_source(image_source) # Need original size for simplify
            ...
            if 'stitched_text_sequential' not in simplified_result:
                 logging.error("Failed to extract 'stitched_text_sequential' during simplification.")
                 if 'error' in simplified_result:
                      raise LensParsingError(f"Error during simplification: {simplified_result['error']}")
                 else:
                      raise LensParsingError("Unknown error: 'stitched_text_sequential' key missing after simplification.")
