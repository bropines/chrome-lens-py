...
from .constants import MIME_TO_EXT, LENS_UPLOAD_ENDPOINT, LENS_METADATA_ENDPOINT, HEADERS_DEFAULT
from .utils import sleep, is_supported_mime # sleep might be less relevant with async/rate limiting
from .image_processing import resize_image, resize_image_from_buffer
from .cookies_manager import CookiesManager
from .exceptions import LensError, LensAPIError, LensParsingError
...
class LensCore:
    ...
    def __init__(self, config=None, sleep_time=1000, logging_level=logging.WARNING):
        ...
        logging.debug(f"Rate limiting configured to max {self.max_requests_per_minute} RPM")
    ...
    def setup_client(self):
        ...
        if self.cookies_manager.cookies:
             initial_cookies = {v['name']: v['value'] for k, v in self.cookies_manager.cookies.items()}
    ...
    def get_headers(self, step='upload', referer=None):
        ...
        if step == 'metadata':
            ...
            metadata_headers.update({
                ...
                'Referer': referer if referer else HEADERS_DEFAULT.get('Referer'), # Use dynamic referer
    ...
    async def _rate_limit_wait(self):
        ...
        if len(self.request_timestamps) >= self.max_requests_per_minute:
            time_to_wait = 60 - (now - self.request_timestamps[0]) + 0.1 # Wait until the oldest request expires + buffer
            logging.debug(f"Rate limit reached ({len(self.request_timestamps)}/{self.max_requests_per_minute} RPM). Waiting for {time_to_wait:.2f} seconds.")
    ...
    def _extract_ids_from_url(self, url_string):
        ...
        try:
            ...
            logging.debug(f"Extracted vsrid: {vsrid}, lsessionid: {lsessionid} from URL.")
    ...
    def _adaptive_parse_text_and_language(self, metadata_json):
        ...
        try:
            ...
            if not isinstance(metadata_json, list) or not metadata_json:
                logging.error("Invalid JSON structure: metadata_json is not a non-empty list.")
                raise LensParsingError("Invalid JSON structure: metadata_json is not a non-empty list.")

            # Find the main response container
            response_container = next((item for item in metadata_json if isinstance(item, list) and item and item[0] == "fetch_query_formulation_metadata_response"), None)
            if response_container is None:
                logging.error("Could not find 'fetch_query_formulation_metadata_response' container in metadata.")
                raise LensParsingError("Could not find 'fetch_query_formulation_metadata_response' container.")
            ...
            try:
                ...
                if len(response_container) > 2 and isinstance(response_container[2], list):
                    ...
                    if language:
                        ...
                        logging.warning("Language code not found in expected format within lang_section.")
                else:
                     logging.warning("Language section (index 2) not found or not a list.")
            except (IndexError, TypeError, StopIteration):
                logging.warning("Error or structure mismatch during language code extraction.")
            ...
            for i, path_func in enumerate(possible_paths_to_segments_list):
                ...
                try:
                    ...
                    if isinstance(candidate_iterable, list) and candidate_iterable and isinstance(candidate_iterable[0], list):
                        try:
                            first_segment = candidate_iterable[0]
                            if (len(first_segment) > 1 and isinstance(first_segment[1], list) and
                                first_segment[1] and isinstance(first_segment[1][0], list) and
                                len(first_segment[1][0]) > 0 and isinstance(first_segment[1][0][0], list)):
                                segments_iterable = candidate_iterable
                                logging.debug(f"Text segments list identified at path ending with {path_name}")
                                break # Found a suitable path
                        except (IndexError, TypeError):
                            pass # Structure doesn't match deep checks, try next path
                except (IndexError, TypeError):
                    logging.debug(f"Path ending with {path_name} not found or invalid structure.")
                    pass # Path doesn't exist or structure is wrong, try next path

            if segments_iterable is None:
                logging.error(f"Could not identify valid text segments list using common paths {path_names}.")
            ...
            for segment_list in segments_iterable:
                ...
                last_word_ends_with_space = True # Assume start doesn't need leading space

                if not isinstance(segment_list, list):
                    logging.warning(f"Skipping segment: Expected list, got {type(segment_list)}.")
                    continue
                    ...
                    if len(segment_list) > 1 and isinstance(segment_list[1], list):
                        ...
                        for group_count, word_group in enumerate(word_groups_list, 1):
                            try:
                                ...
                                if (isinstance(word_group, list) and len(word_group) > 0 and
                                    isinstance(word_group[0], list) and isinstance(word_group[0][0], list)):

                                    word_list = word_group[0] # Get the list of words in this group

                                    # Add space between groups if needed
                                    if group_count > 1 and block_text_builder.tell() > 0 and not last_word_ends_with_space:
                                    ...
                                    for word_info in word_list:
                                        try:
                                            ...
                                            if (isinstance(word_info, list) and len(word_info) > 3 and
                                                    isinstance(word_info[1], str) and # Text
                                                    isinstance(word_info[2], str) and # Space indicator (" " or "")
                                                    isinstance(word_info[3], list) and word_info[3] and # Bbox container list
                                                    isinstance(word_info[3][0], list)): # Actual Bbox list

                                                text = word_info[1]
                                                space_indicator = word_info[2] # Usually " " or ""
                                                bbox = word_info[3][0] # Extract the bounding box list
                                                ...
                                                if len(bbox) > 4 and isinstance(bbox[4], (int, float)):
                                                    try:
                                                        ...
                                                        annotation = {"text": text, "bbox": processed_bbox, "angle_degrees": degrees}
                                                    except TypeError:
                                                        logging.warning(f"Could not convert angle {bbox[4]} to degrees for word '{text}'. Skipping degree conversion.")
                                                        annotation = {"text": text, "bbox": processed_bbox}
                                                else:
                                                     annotation = {"text": text, "bbox": processed_bbox} # No angle or invalid

                                                current_block_word_annotations.append(annotation)

                                                # Reconstruct text block
                                                block_text_builder.write(text)
                                                block_text_builder.write(space_indicator)
                                                last_word_ends_with_space = (space_indicator == " ")

                                        except (IndexError, TypeError) as e_word:
                                            logging.warning(f"Skipping word due to structure error or missing elements: {e_word} in word_info: {word_info}")
                                            pass # Skip malformed word info
                            except (IndexError, TypeError) as e_group:
                                logging.warning(f"Skipping word group due to structure error: {e_group} in word_group: {word_group}")
                                pass # Skip malformed word group
                    else:
                        logging.warning(f"Word groups list (index 1) not found or invalid in segment: {segment_list}")
                except (IndexError, TypeError) as e_segment:
                    logging.error(f"Error processing segment structure: {e_segment} in segment_list: {segment_list}")
                except Exception as e_seg_generic:
                    logging.error(f"Unexpected error processing segment: {e_seg_generic}", exc_info=True)


                reconstructed_text = block_text_builder.getvalue().rstrip(' ') # Remove trailing space if any
                ...
                if reconstructed_text or current_block_word_annotations:
                    ...
                    all_word_annotations.extend(current_block_word_annotations) # Collect annotations from all blocks

        except LensParsingError:
            raise # Propagate specific parsing errors
        except Exception as e_main:
            logging.error(f"Critical error during adaptive text extraction: {e_main}", exc_info=True)
            # Raise a generic parsing error if something unexpected happened
            raise LensParsingError(f"Unexpected error during metadata parsing: {e_main}") from e_main

        logging.info(f"Adaptive parsing complete. Language: '{language}'. Text blocks found: {len(reconstructed_blocks)}. Total word annotations: {len(all_word_annotations)}.")
        # Return the structured data
        return {
            "language": language if language else "und", # Default to 'undetermined' if not found
    ...
    async def _perform_scan(self, image_data, mime_type, dimensions):
        ...
        try:
            response_upload = await self.client.post(upload_url_obj, headers=upload_headers, files=files)
            logging.debug(f"Upload Response Status: {response_upload.status_code}")
            logging.debug(f"Upload Response URL (final after redirects): {response_upload.url}")
            ...
            logging.error(f"HTTP error during image upload: {e.response.status_code} for URL {e.request.url}")
            ...
            raise LensAPIError(f"Image upload failed with status {e.response.status_code}", e.response.status_code, e.response.headers, await e.response.aread()) from e
        ...
        if not vsrid or not lsessionid:
            logging.error("Failed to extract vsrid or lsessionid from upload redirect URL.")
            self.cookies_manager.save_cookies() # Save cookies obtained so far
            raise LensParsingError("Failed to get session IDs from upload response URL.", body=final_url)
        ...
        try:
            response_metadata = await self.client.get(metadata_url_obj, headers=metadata_headers)
            ...
            logging.error(f"HTTP error during metadata fetch: {e.response.status_code} for URL {e.request.url}")
            logging.debug(f"Response Headers: {e.response.headers}")
            logging.debug(f"Response Body: {await e.response.aread()}")
            raise LensAPIError(f"Metadata fetch failed with status {e.response.status_code}", e.response.status_code, e.response.headers, await e.response.aread()) from e
        except httpx.RequestError as e:
        ...
        if self.debug_out:
            try:
                ...
                logging.warning(f"Could not save debug output to {self.debug_out}: {e_debug}")
        ...
        except (json.JSONDecodeError, json5.JSONDecodeError, LensParsingError) as e_parse:
            logging.error(f"Error parsing JSON or extracting text from metadata: {e_parse}", exc_info=True)
            raise LensParsingError(f"Failed to parse metadata response: {e_parse}", body=response_text) from e_parse
        except Exception as e_generic:
             logging.error(f"Unexpected error processing metadata: {e_generic}", exc_info=True)
             raise LensError(f"Unexpected error processing metadata: {e_generic}") from e_generic
...
class Lens(LensCore):
    ...
    def __init__(self, config=None, sleep_time=1000, logging_level=logging.WARNING):
        ...
        logging.getLogger().setLevel(self.logging_level) # Ensure level is set in this higher class too
    ...
    async def scan_by_file(self, file_path):
        ...
        try:
            ...
            logging.debug(f"Image resized to: {dimensions}, original size: {original_size}")
        except Exception as e:
    ...
    async def scan_by_url(self, url):
        ...
        try:
            ...
            async with self.client as client_for_download: # Use existing client configuration
                 ...
                 content_type = response.headers.get('Content-Type', 'image/jpeg') # Get content type if available
                 logging.debug(f"Image downloaded from {url}, content type: {content_type}")
                 # Now scan the downloaded buffer
                 return await self.scan_by_buffer(buffer, content_type_hint=content_type)
        except httpx.HTTPStatusError as e:
            logging.error(f"Failed to download image from URL {url}: {e.response.status_code}")
            raise LensError(f"Failed to download image from URL: {url} (Status: {e.response.status_code})") from e
        except httpx.RequestError as e:
            logging.error(f"Network error downloading image from URL {url}: {e}")
            raise LensError(f"Network error downloading image from URL: {url}: {e}") from e
        except Exception as e:
    ...
    async def scan_by_buffer(self, buffer, content_type_hint='image/jpeg'):
        ...
        try:
            ...
            logging.debug(f"Image from buffer resized to: {dimensions}, original size: {original_size}")
        except Exception as e:
        ...
        content_type = content_type_hint if content_type_hint in MIME_TO_EXT else 'image/jpeg'
