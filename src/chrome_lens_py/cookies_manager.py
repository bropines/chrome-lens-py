...
class CookiesManager:
    def __init__(self, config=None, cookie_file=None, logging_level=logging.WARNING):
        ...
        if cookies_from_config:
            self.cookies = {} # Clear previously loaded cookies if new ones are provided
            if isinstance(cookies_from_config, str):
                if os.path.isfile(cookies_from_config):
                    self.import_cookies_from_file(cookies_from_config) # Assumes Netscape format for file path in config
                else:
            ...
            logging.debug("Imported cookies specified in config, overwriting any loaded from file.")
            ...
            try:
                with open(self.cookie_file, 'rb') as f:
                    ...
                    if isinstance(loaded_cookies, dict):
                        ...
                        logging.debug(f"Loaded {len(self.cookies)} cookies from file: {self.cookie_file}")
                    else:
                        logging.warning(f"Invalid cookie data type found in {self.cookie_file}, expected dict. Ignoring.")
                        self.cookies = {}
            except (FileNotFoundError, pickle.PickleError, EOFError) as e:
                logging.warning(f"Error loading cookies from file {self.cookie_file}: {e}. Starting with empty cookies.")
                self.cookies = {}
            except Exception as e:
                 logging.error(f"Unexpected error loading cookies from {self.cookie_file}: {e}. Starting with empty cookies.", exc_info=True)
                 self.cookies = {}
        else:
            logging.debug(f"Cookie file not found: {self.cookie_file}. Starting with empty cookies.")
            self.cookies = {}
    ...
    def parse_cookie_dict(self, cookie_dict):
        ...
        for key, cookie_data in cookie_dict.items():
            if isinstance(cookie_data, dict) and 'value' in cookie_data:
                ...
                logging.warning(f"Skipping invalid cookie entry for key '{key}': {cookie_data}")
            ...
            with open(file_path, 'r', encoding='utf-8') as file:
                for line in file:
                    ...
                    try:
                        ...
                        if len(parts) == 7:
                        ...
                        elif len(parts) == 6: # Handle files without the initial domain flag correctly
                            ...
                            logging.warning(f"Cookie line has 6 parts, check format: {line}")
                            # Attempt to parse anyway if possible, might be incorrect
                            # self.cookies[name] = { ... } # Decide if you want to handle 6-part lines
                        else:
                             logging.warning(f"Skipping malformed cookie line (expected 7 tab-separated parts): {line}")
                    except IndexError as e:
                        logging.error(f"Error parsing cookie line elements: {line.strip()} - {e}")
                    except ValueError as e:
                         logging.error(f"Error converting expires timestamp on line: {line.strip()} - {e}")
        except (FileNotFoundError, IOError) as e:
            raise LensCookieError(f"Error reading Netscape cookie file {file_path}: {e}") from e
        except Exception as e:
            raise LensCookieError(f"Unexpected error parsing Netscape cookie file {file_path}: {e}") from e
    ...
    def generate_cookie_header(self):
        ...
        for cookie in self.cookies.values():
             ...
             if 'name' in cookie and 'value' in cookie and cookie['value'] is not None:
                  ...
                  logging.warning(f"Skipping cookie with missing name or value: {cookie.get('name', 'N/A')}")
    ...
    def filter_expired_cookies(self):
        ...
        try:
            ...
            self.cookies = {
                ...
                if v.get('expires') is None or v['expires'] == 0 or v['expires'] > current_time
            }
            filtered_count = len(self.cookies)
            if initial_count != filtered_count:
                 logging.debug(f"Filtered {initial_count - filtered_count} expired cookies.")
        except TypeError as e:
            # This might happen if 'expires' is not a comparable type (e.g., unexpected string)
            logging.error(f"Error filtering cookies due to invalid 'expires' data: {e}. Attempting rewrite.")
            # Consider resetting or trying to fix cookies instead of just rewriting
            # For now, let's log the problematic cookies
            for k, v in self.cookies.items():
                if v.get('expires') is not None and not isinstance(v['expires'], (int, float, type(None))):
                     logging.error(f"Problematic cookie expires value for key '{k}': {v['expires']} (type: {type(v['expires'])})")
            ...
            for k, v in self.cookies.items():
                 ...
                 if expires is None or expires == 0 or isinstance(expires, (int, float)) and expires > current_time:
                      fixed_cookies[k] = v
                 else:
                      logging.warning(f"Removing cookie '{k}' due to invalid or expired 'expires' value: {expires}")
    ...
    def update_cookies(self, cookie_jar):
        ...
        """
        logging.debug(f"Attempting to update cookies from object of type: {type(cookie_jar)}")
        updated = False
                ...
                if cookie.expires is not None:
                    ...
                    try:
                        expires_timestamp = float(cookie.expires)
                         ...
                         logging.warning(f"Could not parse expires timestamp '{cookie.expires}' for cookie '{cookie.name}'. Setting to None.")

                # Check if cookie changed or is new
                current_cookie = self.cookies.get(cookie.name)
                new_value = cookie.value
                if not current_cookie or current_cookie.get('value') != new_value or current_cookie.get('expires') != expires_timestamp:
        ...
        elif isinstance(cookie_jar, str): # Handle Set-Cookie header string (less likely now)
            logging.debug(f"Updating cookies from Set-Cookie header string: {cookie_jar[:100]}...")
            try:
             ...
             logging.warning(f"Unsupported cookie_jar type for update: {type(cookie_jar)}")
    ...
    def save_cookies(self):
        ...
        if not os.path.exists(cookie_dir):
             try:
                 ...
                 logging.error(f"Failed to create directory {cookie_dir} for saving cookies: {e}. Cannot save cookies.")
                 return # Abort saving

        try:
            with open(self.cookie_file, 'wb') as f:
                pickle.dump(self.cookies, f)
                logging.debug(f"Saved {len(self.cookies)} cookies to file: {self.cookie_file}")
        except (IOError, pickle.PickleError) as e:
            logging.error(f"Error saving cookies to file {self.cookie_file}: {e}")
        except Exception as e:
            logging.error(f"Unexpected error saving cookies to {self.cookie_file}: {e}", exc_info=True)
    ...
    def rewrite_cookies(self):
        ...
        """
        logging.warning("Rewriting cookies: Clearing current state and reloading from config/file.")
        self.cookies = {}
        self.imported_cookies = False # Reset flag to allow reloading
        # Re-run the initialization logic for loading/importing
        self.__init__(config=self.config, cookie_file=self.cookie_file, logging_level=self.logging_level)
    ...
    def _parse_expires(self, expires_input):
        ...
        if isinstance(expires_input, str):
            try:
                ...
                expires_input_gmt = expires_input.replace("GMT", "").strip() # Remove GMT for strptime
                if not expires_input_gmt.endswith("Z"): # Ensure timezone info consistency if needed
                ...
                for fmt in common_formats:
                    try:
                        ...
                        dt = datetime.strptime(expires_input_gmt, fmt.replace("%Z", "GMT"))
                        # Convert naive datetime assumed to be GMT to timestamp
                        timestamp = dt.replace(tzinfo=datetime.timezone.utc).timestamp()
                        logging.debug(f"Parsed date string '{expires_input}' to timestamp: {timestamp}")
                        return float(timestamp)
                    except ValueError:
                        continue # Try next format
                # If all formats fail
                logging.warning(f"Failed to parse expires string '{expires_input}' into a known date format or timestamp.")
                return None
        # If input is not a recognized type
        logging.warning(f"Unparseable expires value type: {type(expires_input)}, value: {expires_input}")
        return None
