...
# Import exceptions
from .exceptions import LensAPIError, LensParsingError, LensCookieError, LensError
# Keep utils
...
def print_help():
    ...
    console.print(
        "[b]full_text_default[/b]         Get the main reconstructed text") # Reverted Name
    console.print(
        "[b]full_text_old_method[/b]      Get text stitched sequentially") # Reverted Name
    console.print(
        "[b]full_text_new_method[/b]      Get text stitched using line reconstruction") # Reverted Name
    console.print(
...
def load_config(config_file=None):
    ...
    if not effective_config_file:
        ...
        if env_config_path and os.path.isfile(env_config_path):
            ...
            logging.debug(f"Using config file from environment variable: {effective_config_file}")
        else:
    ...
    if effective_config_file and os.path.isfile(effective_config_file):
        try:
            ...
            console.print(f"[red]Error loading config file '{effective_config_file}':[/red] {e}")
            # Decide if execution should stop or continue with defaults
            # sys.exit(1) # Optional: exit on config load error
        except Exception as e:
             console.print(f"[red]Unexpected error loading config file '{effective_config_file}':[/red] {e}")
...
def save_config(config):
    ...
    if not os.path.exists(config_dir):
        try:
            ...
            console.print(f"[red]Error creating config directory '{config_dir}':[/red] {e}. Cannot save config.")
            return
    default_config_file = os.path.join(config_dir, 'config.json')
    try:
        ...
        console.print(f"[red]Error saving config file '{default_config_file}':[/red] {e}")
    except Exception as e:
         console.print(f"[red]Unexpected error saving config file '{default_config_file}':[/red] {e}")
...
async def process_image(image_source, data_type, coordinate_format, api):
    ...
    try:
        ...
        if data_type == "all":
            ...
            result = await api.get_full_text(image_source) # Maps to the new get_full_text
    ...
    # Catch errors specifically from the API layer or below
        ...
        if logging.getLogger().isEnabledFor(logging.DEBUG) and hasattr(e, 'body'):
             console.print(f"[grey]Response Body (partial): {str(e.body)[:200]}...[/grey]")
        return None # Indicate failure
    except Exception as e:
        # Catch unexpected errors during the API call
        logging.error(f"Unexpected Error processing '{str(image_source)[:100]}...': {e}", exc_info=True)
...
async def process_directory(directory_path, data_type, coordinate_format, api, out_txt_option=None):
    ...
    console.print(f"Found {len(files_to_process)} supported images in '{directory_path}'. Processing...")
    ...
    if out_txt_option and out_txt_option != 'per_file':
        ...
        try:
            ...
            console.print(f"[red]Error opening output file '{output_file_path}': {e}. Cannot write output.[/red]")
    ...
    for idx, (filename, file_path) in enumerate(files_to_process):
        if logging.root.level > logging.INFO: # Add visual separator if not in verbose modes
            console.print(f"--- Processing file {idx+1}/{len(files_to_process)}: [cyan]{filename}[/cyan] ---")
        else:
             logging.info(f"Processing file {idx+1}/{len(files_to_process)}: {filename}...")
        ...
        if result is not None:
            ...
            if out_txt_option == 'per_file':
                ...
                try:
                    ...
                    console.print(f"[red]Error writing output file '{output_file_path}': {e}[/red]")
            ...
            # Processing failed for this file, error already logged by process_image
             ...
             results_data[filename] = {"error": "Processing failed, see logs."} # Mark failure
...
async def main():
    ...
    parser.add_argument('data_type', nargs='?', default='all', choices=[
        'all', 'full_text_default', 'full_text_old_method', 'full_text_new_method', 'coordinates'],
        help="Type of data to extract (default: all). See --help for details.")
    # Options
    parser.add_argument('-h', '--help', action='store_true',
                        help="Show this help message and exit.")
    parser.add_argument('-c', '--cookie-file',
                        help="Path to the Netscape or .pkl cookie file.")
    ...
    parser.add_argument('--coordinate-format', choices=['percent', 'pixels'], default=None, # Default handled later
    ...
    parser.add_argument('--debug-out',
                        help="Path to save raw metadata response text for debugging.")
    ...
    parser.add_argument('--rate-limit-rpm', type=int, default=None, # Default handled later
    ...
    if args.rate_limit_rpm is not None:
        ...
        final_config.setdefault('rate_limiting', {})['max_requests_per_minute'] = args.rate_limit_rpm
    ...
    logging.debug(f"Final configuration before API init: {json.dumps(final_config, default=str)}")

    # --- Update Default Config File (if requested) ---
    if args.update_config and not args.config_file: # Only update default if no specific file was loaded
        ...
        update_fields = ['coordinate_format', 'debug', 'data_type', 'rate_limiting', 'debug_out'] #'sleep_time', 'header_type' removed
        for field in update_fields:
            ...
            if field == 'rate_limiting':
                 current_rpm = current_default_config.get('rate_limiting', {}).get('max_requests_per_minute')
                 cli_rpm = cli_value.get('max_requests_per_minute') if isinstance(cli_value, dict) else None
                 if cli_rpm is not None and current_rpm != cli_rpm:
                      current_default_config.setdefault('rate_limiting', {})['max_requests_per_minute'] = cli_rpm
                      config_updated = True
            elif field == 'data_type': # data_type comes from args directly for update check
                 if args.data_type != current_default_config.get(field): # Compare CLI arg with current config value
                      current_default_config[field] = args.data_type # Update config with CLI value
                      config_updated = True
            elif cli_value is not None and current_default_config.get(field) != cli_value:
    ...
    try:
        ...
        coordinate_format = final_config['coordinate_format'] # Use final determined format
        ...
        if os.path.isdir(image_source):
            ...
            if result is not None and not args.out_txt: # Print only if no file output for single image
                # Format result nicely for console
                if isinstance(result, dict) or isinstance(result, list):
                     console.print(json.dumps(result, indent=2, ensure_ascii=False))
                else:
                     console.print(str(result))
            elif result is not None and args.out_txt: # Handle single file output with --out-txt
                 output_filename = args.out_txt if args.out_txt != 'per_file' else f"{os.path.splitext(os.path.basename(image_source))[0]}.txt"
                 ...
                 if is_url(image_source):
                     ...
                     output_dir = os.path.dirname(os.path.abspath(image_source)) if os.path.isfile(image_source) else os.path.abspath(image_source)
                 ...
                 try:
                      ...
                      console.print(f"[red]Error writing output to file '{output_path}': {e}[/red]")

        else:
            console.print(f"[red]Error:[/red] Image source '{image_source}' is not a valid file, directory, or URL.")
            sys.exit(1)

    except (LensAPIError, LensParsingError, LensCookieError, LensError) as e:
        # Catch errors raised from API initialization or processing
        logging.critical(f"Critical API Error: {e}", exc_info=logging_level <= logging.DEBUG) # Show traceback on debug
        console.print(f"[bold red]Critical Error:[/bold red] {e}")
        sys.exit(1)
    except Exception as e:
        # Catch unexpected errors in the main flow
        logging.critical(f"An unexpected error occurred in main: {e}", exc_info=True) # Always show traceback
