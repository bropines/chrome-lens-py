...
if TYPE_CHECKING:
    from ..utils.lens_betterproto import LensOverlayServerResponse, LensOverlayClusterInfo, LensOverlayRoutingInfo
else:
    from ..utils.lens_betterproto import LensOverlayServerResponse, LensOverlayClusterInfo, LensOverlayRoutingInfo
    ...
    async def send_request(
        ...
        async with httpx.AsyncClient(**self.proxy_settings, http2=True) as client:
            try:
                ...
                raise LensAPIError(f"Ошибка сети или запроса (возможно, связана с прокси): {e_req}") from e_req
                ...
                raise LensAPIError(f"Unexpected error while executing the request: {e_gen}")