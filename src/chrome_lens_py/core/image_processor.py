    ...
    if isinstance(image_source, str):
        if is_url(image_source):
            ...
            try:
                ...
                raise LensImageError(f"Error opening file path '{image_source}': {e}") from e
...
def _resize_and_serialize_pil_image(pil_image: Image.Image) -> Tuple[bytes, int, int]:
    ...
    if pil_image.width > DEFAULT_IMAGE_MAX_DIMENSION or pil_image.height > DEFAULT_IMAGE_MAX_DIMENSION:
        pil_image.thumbnail((DEFAULT_IMAGE_MAX_DIMENSION, DEFAULT_IMAGE_MAX_DIMENSION), Image.Resampling.LANCZOS)
...
async def prepare_image_for_api(image_source: Any) -> Tuple[bytes, int, int, Image.Image]:
    """
...
def draw_overlay_on_image(
    ...
    for norm_x1, norm_y1, norm_x2, norm_y2 in ocr_boxes_norm:
        draw.rectangle([int(norm_x1 * img_width), int(norm_y1 * img_height), int(norm_x2 * img_width), int(norm_y2 * img_height)], fill=fill_color)
    ...
    px_overall_x1, px_overall_y1 = int(overall_ocr_min_x * img_width), int(overall_ocr_min_y * img_height)
    px_overall_x2, px_overall_y2 = int(overall_ocr_max_x * img_width), int(overall_ocr_max_y * img_height)
    
    overlay_width_px = px_overall_x2 - px_overall_x1
    if overlay_width_px <= 0: return img_draw
    
    padding = 4 
    available_width_for_text = overlay_width_px - 2 * padding
    if available_width_for_text <= 0: return img_draw
    ...
    for word in translated_text.split():
        ...
        try:
            ...
            else: line_width = draw.textbbox((0, 0), test_line, font=font)[2]
        except Exception: line_width = len(test_line) * (font.size if hasattr(font, 'size') else 10) * 0.6
        
        if line_width <= available_width_for_text:
            current_line = test_line
        else:
            if current_line: lines_to_draw.append(current_line)
            current_line = word
    if current_line: lines_to_draw.append(current_line)
    ...
    for line_str in lines_to_draw:
        try:
            if hasattr(draw, "textbbox"):
                ...
                line_height, line_width = bbox[3] - bbox[1], bbox[2] - bbox[0]
            else:
                size_approx = font.size if hasattr(font, 'size') else 10
                line_height, line_width = size_approx, len(line_str) * size_approx * 0.6
            
            if current_y + line_height > px_overall_y2 - padding: break
            
            pos_x = px_overall_x1 + (overlay_width_px - line_width) / 2
            try: draw.text((pos_x, current_y), line_str, fill=text_color, font=font, anchor="lt")
            except TypeError: draw.text((pos_x, current_y), line_str, fill=text_color, font=font)
